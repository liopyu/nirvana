WorldgenEvents.remove(event => {
  ...
  /*WorldgenEvents.add(event => {
    ...
    event.addOre(ore => {
      ore.id = 'kubejs:air' // (optional, but recommended) custom id for the feature
      ...
      ore.addTarget([
        'minecraft:brewing_stand',     // or any kind of dirt (including coarse, rooted, etc.)...
      ], 'minecraft:air')       // with TNT (trust me, it'll be funny)
      ...
      ore.size = 1                      // max. vein size
      ore.noSurface = 0                 // chance to discard if the ore would be exposed to air
      ore.worldgenLayer = 'raw_generation'  // what generation step the ores should be generated in (see below)
