...
ItemEvents.rightClicked('magicmirror:magicmirror', event => {
    ...
    server.runCommandSilent(`execute as ${player.username} run playsound minecraft:teleport ambient @s ${player.x} ${player.y} ${player.z} 1 1`)
})
...
ItemEvents.rightClicked(event => {
    ...
    if (item.id == 'sophisticatedbackpacks:backpack') {
        server.runCommandSilent(`title ${player.username} actionbar {"text":"I think I should place this down to open it.","bold":true,"color":"green"}`);
        event.cancel()
    }
    if (item.id == 'sophisticatedbackpacks:iron_backpack') {
        server.runCommandSilent(`title ${player.username} actionbar {"text":"I think I should place this down to open it.","bold":true,"color":"green"}`);
        event.cancel()
    }
    if (item.id == 'sophisticatedbackpacks:gold_backpack') {
        server.runCommandSilent(`title ${player.username} actionbar {"text":"I think I should place this down to open it.","bold":true,"color":"green"}`);
        event.cancel()
    }
    if (item.id == 'sophisticatedbackpacks:diamond_backpack') {
        server.runCommandSilent(`title ${player.username} actionbar {"text":"I think I should place this down to open it.","bold":true,"color":"green"}`);
        event.cancel()
    }
    if (item.id == 'sophisticatedbackpacks:netherite_backpack') {
        server.runCommandSilent(`title ${player.username} actionbar {"text":"I think I should place this down to open it.","bold":true,"color":"green"}`);
        event.cancel()
    }
